...
// ------------------------------------------------------------------
// 1. DATA PRODUK (LENGKAP DARI KODE PERTAMA)
// ------------------------------------------------------------------
...
// Tentukan tier R2 berdasarkan harga
function getR2Tier(price) {
    if (price <= 80000) return 'hemat';
    if (price <= 120000) return 'populer';
    return 'premium';
}

// ------------------------------------------------------------------
// 3. STATE GLOBAL (Catalog, Filter, Pagination, Wishlist, Cart)
// ------------------------------------------------------------------
...
// Wishlist & Cart (gunakan localStorage)
...
// ------------------------------------------------------------------
// 4. FUNGSI KATALOG (Switch, Filter, Sort, Search, Pagination)
// ------------------------------------------------------------------
...
function getProcessedProducts() {
    ...
    if (activeCatalog === 'r2') {
        if (activeFilter !== 'all') {
            r = r.filter(function(p) {
                return getR2Tier(p.price) === activeFilter;
            });
        }
    } else {
        if (activeFilter !== 'all') {
...
function renderProductDisplay() {
    ...
    if (!pp.length) {
        var np = document.getElementById('noProductFound');
...
// ------------------------------------------------------------------
// 5. EVENT HANDLERS (Filter, Sort, Pagination, Search)
// ------------------------------------------------------------------
...
// Search handler (bisa dipanggil dari input onkeyup)
...
// ------------------------------------------------------------------
// 6. FUNGSI WISHLIST & CART (Minimal, dapat dikembangkan lebih lanjut)
// ------------------------------------------------------------------
...
function toggleWishlistItem(id, event) {
    ...
    saveWishlist();
    renderProductDisplay(); // refresh tampilan
    // Optional: update heart icon tanpa reload (bisa ditingkatkan)
}
...
// ------------------------------------------------------------------
// 7. INTEGRASI window.R2Data (sesuai permintaan)
// ------------------------------------------------------------------
...
window.R2Data = (function() {
    ...
    return {
        ...
        getProductsR2: function() { return productsR2; },
        getProductsResmi: function() { return productsResmi; },
        formatRupiah: formatRupiah,
        // Tambahan: akses state jika diperlukan
...
// Pastikan elemen DOM sudah ada sebelum menjalankan render
// Jika kode ini dijalankan sebelum DOM siap, gunakan event DOMContentLoaded
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Set active tab awal
        document.querySelectorAll('.catalog-tab').forEach(function(tab) {
            if (tab.dataset.tab === activeCatalog) {
